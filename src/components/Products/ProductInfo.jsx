import React from "react";

function ProductInfo({ mainImage, title, description, price, discountPercentage }) {
  const discountedPrice = (price * (1 - discountPercentage / 100)).toFixed(2);

  return (
    <>
      {/* Show image on mobile only */}
      <img src={mainImage} alt="Product" className="block md:hidden w-full rounded-xl mb-6" />

      <p className="uppercase text-orange-500 font-bold text-sm tracking-widest mb-3">Sneaker Company</p>

      <h1 className="text-4xl font-bold text-gray-900 mb-6 leading-tight">{title}</h1>

      <p className="text-gray-600 text-base mb-6 leading-relaxed">{description}</p>

      <div className="mb-6">
        <div className="flex items-center gap-4">
          <span className="text-3xl font-bold text-gray-900">${discountedPrice}</span>
          <span className="bg-orange-100 text-orange-500 font-bold text-sm px-2 py-1 rounded-lg">
            {Math.round(discountPercentage)}%
          </span>
        </div>
        <p className="text-gray-400 line-through mt-1">${price}</p>
      </div>
    </>
  );
}

export default ProductInfo;